...
状态：已完成。当前支持追加导入和覆盖导入；只导入预览中可导入的行，错误行可打开详情核对。保存失败时不替换内存列表、不刷新 UI 数据。

- 支持追加导入。
- 支持覆盖导入，覆盖前保留最近备份。
- 显示错误行详情，错误行不会导入。
- `FoodStore.saveFoodsForImport()` 使用同步提交结果；提交失败时 `MainActivity` 不替换当前 `foods`。
- 视觉 QA 已覆盖首页导入按钮、系统文件选择器、导入预览、错误行详情、覆盖确认、覆盖导入结果和 debug-only 强制失败回滚。
...
### OCR-002：CameraX + ML Kit POC / 视频回放模拟

状态：APK POC 已完成，且 2026-07-06 已新增视频回放模拟相机路径。当前入口为 `识别包装文字`，页面只输出 raw OCR、候选摘要和表单草稿，不直接保存食品。

- 引入 CameraX Preview + ImageAnalysis。
- 引入 ML Kit Text Recognition v2 Chinese。
- 使用 `STRATEGY_KEEP_ONLY_LATEST`，避免积压帧。
- 新增 `video/*` 文件选择，使用 `MediaMetadataRetriever` 抽帧并复用同一套 ML Kit OCR、候选提取和多帧投票逻辑。
- 输出 raw OCR text，不保存食品。
- 视觉 QA 已覆盖视频选择、视频帧预览、raw OCR 刷新、无稳定候选时禁用“使用候选”；真实相机实时预览按当前用户要求先不做。
...
## 9. 2026-07-05 / 2026-07-06 OCR-002 落地更新
...
- 2026-07-06 新增视频模拟入口：用户可选择本地视频，APK 按固定间隔抽帧、显示帧画面、运行 ML Kit OCR，并复用 `DateOcrFrameVoter` 输出稳定候选或低置信度提示。

仍未声明完成的内容：

- 当前视觉 QA 已覆盖首页“识别包装文字”入口、无可用相机时的 OCR 页面降级态、视频选择、视频帧预览和 raw OCR 更新；真实相机实时预览按用户 2026-07-06 最新要求先不做。
- 视频样本 1 未形成稳定生产日期候选，因此真实包装识别准确率仍是后续优化项，不能用链路通过替代准确率验收。
