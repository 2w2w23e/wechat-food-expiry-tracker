...
当前 Android APK 本地版已支持统一智能识别、商品码识别、商品信息查询、包装文字 OCR 候选、本地提醒、系统通知和已用完归档。后续最重要的方向是继续扩大真实包装样本回归，提升生产日期和保质期识别准确率。
...
- APK 本地版支持统一智能识别：同一入口融合条码、中文/拉丁包装文字、低对比激光喷码和日期候选，结果确认后才进入新增表单
...
当前版本：APK 本地版 V0.3.9 智能/固定提醒模式 + 中文包装喷码与保质期容错识别 + 高清对焦相机分析 + 已封存的小程序 V0。
...
本地 ML Kit 条码和包装文字 OCR 只生成候选，必须由用户确认后才可填入新增表单。
