...
当前 Android APK 本地版已支持统一智能识别、商品码识别、商品信息查询、多模型包装文字 OCR 候选、本地提醒、系统通知和已用完归档。OCR 采用 ML Kit 中文/拉丁文字与条码模型、OpenCV 低对比增强、ONNX Runtime Android 1.27.0 驱动的 `PP-OCRv6_rec_small` 行识别模型，以及日期规则和跨帧投票协同；后续仍需持续扩大真实包装样本回归，不能把单个样本可读等同于整体准确率达标。
...
- APK 本地版支持统一智能识别：同一入口融合 ML Kit 中文/拉丁包装文字与条码、OpenCV 低对比增强、PP-OCRv6 日期行识别和跨帧日期候选，结果确认后才进入新增表单
- 商品名智能选择：条码商品库命中优先，其次要求明确“产品名称/品名”标签，或同时满足食品语义、文字结构与跨帧稳定；最多展示 1 个最佳高可信名称，说明书、医疗文字、公司名、混合脚本乱码和低分噪声不展示
...
当前版本：APK 本地版 V0.3.10 多模型本地 OCR + 常见包装日期规则 + 智能/固定提醒模式 + 已封存的小程序 V0。
...
本地条码与多模型包装文字 OCR 只生成候选，必须由用户确认后才可填入新增表单，绝不直接保存到食品列表。常见日期标签、格式、推断边界和样本验收状态见 `docs/APK_OCR_DATE_FORMAT_MATRIX.md`。

V0.3.10 的 `minSdk` 为 24。当前 debug APK 约 212 MB，模型只保留当前链路必需的少量文件；离线效果优先，但可交付 APK 必须小于 1 GB。
...
- ONNX Runtime Android 1.27.0 + `PP-OCRv6_rec_small` 日期行识别
- 日期规则、商品名可信门槛与跨帧候选投票
